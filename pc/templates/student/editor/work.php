...
if(isset($_POST['save'])){
    ...
    if(trim($_POST['description']) == ""){
        $_SESSION['error'] = "Job description cannot be blank";
        header("location: ".$_SERVER['REQUEST_URI']);
        exit;
    }
...
<article class="col-4">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>" method="post" accept-charset="utf-8">
        <label for="title">
            <input type="text" name="title" placeholder="Job Title" class="input-container"/>
        </label>
        <label for="employer">
            <input type="text" name="employer" placeholder="Company/Organization" class="input-container" />
        </label>
        <label for="desc">
            <textarea name="description" class="input-container" style="height: 120px;" placeholder="Job Description"></textarea>
        </label>
        <p><b>Dates:</b></p>
        <div class="row">
            <div class="col-3">
